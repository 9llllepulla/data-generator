...
data FullName =
  FullName Name LastName BirthDay
  deriving (Show)

instance Generated FullName where
  toString (FullName name lastName birthDay) =
    capitalize name ++ "," ++ capitalize lastName ++ "," ++ birthDay
...
fullNamesGen range amount offset =
  let numbers = take amount [1 ..]
   in take amount
        $ uniqueFilter . map (\x -> toString $ fullNameGen range (x + offset))
        $ numbers
...
randomAnyName charsAmount offset =
  take charsAmount (randomRs ('a', 'z') $ mkStdGen $ charsAmount * offset)
